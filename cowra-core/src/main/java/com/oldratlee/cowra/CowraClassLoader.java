 ...
 * @author ding.lid
 */
public class CowraClassLoader extends URLClassLoader {
    public static final String CLASS = "class ";
    private final Map<DelegateType, List<String>> delegateConfig;
    private final CowraMatcher matcher;

    public CowraClassLoader(URL[] urls, Map<DelegateType, List<String>> delegateConfig) {
        this(urls, delegateConfig, null, null);
    }

    public CowraClassLoader(URL[] urls, Map<DelegateType, List<String>> delegateConfig, ClassLoader parent) {
        this(urls, delegateConfig, null, parent);
    }

    public CowraClassLoader(URL[] urls, Map<DelegateType, List<String>> delegateConfig, CowraMatcher matcher) {
     ...
     */
    public CowraClassLoader(URL[] urls, Map<DelegateType, List<String>> delegateConfig, CowraMatcher matcher, ClassLoader parent) {
        super(urls, parent == null ? getSystemClassLoader() : parent);

        if (delegateConfig == null) {
            delegateConfig = new EnumMap<>(DelegateType.class);
        }
        this.matcher = matcher == null ? new DefaultCowraMatcher() : matcher;
    ...
    private Class loadClass0(String className) throws ClassNotFoundException {
        ...
        switch (delegateType) {
            case NONE:
                throw new ClassNotFoundException(CLASS + className + "(NONE) is forbidden by cowra config!");
            case PARENT_ONLY:
    ...
    static DelegateType findDelegateTypeInDelegateConfigs(String className, Map<DelegateType, List<String>> delegateConfig, CowraMatcher matcher) {
    ...
    static boolean matchPatterns(String className, List<String> patterns, CowraMatcher matcher) {
    ...
    public String toString() {
        return CowraClassLoader.class.getSimpleName() + "(urls: " + Arrays.toString(getURLs()) + ")";
    }
}
